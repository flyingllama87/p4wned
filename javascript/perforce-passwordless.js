// perforce-passwordless.js
// Finds Perforce user accounts that have no password set.
// Uses tagged output format (tag parameter) to detect absence of the Password field.
// Accounts without passwords allow direct unauthenticated login.
//
// Usage: node perforce-passwordless.js [targets_file]
...
const stats = { total: 0, passwordlessFound: 0, errors: 0 };
...
function queryServer(host, port, useSSL, useUnicode) {
    return new Promise((resolve) => {
        ...
        // tag:"" switches to client-FstatInfo tagged output; Password field absent for passwordless users
        const cmd = [
        ...
        cmd.push(['charset','1'],['utf8bom','1'],['clientCase','0'],['tag',''],['func','user-users']);
...
async function checkTarget(host, port) {
    ...
    // Tagged output: client-FstatInfo with capitalized fields. Password absent = passwordless.
    const passwordless = r.packets.filter(p =>
        p['func'] === 'client-FstatInfo' && p['User'] && p['Email'] && !p['Password']
    );

    if (passwordless.length > 0) {
        stats.passwordlessFound += passwordless.length;
        const proto = useSSL ? 'ssl' : 'tcp';
        passwordless.forEach(u => console.log(`[${label}] [${proto}] [PASSWORDLESS] ${u['User']} <${u['Email']}> "${u['FullName'] || ''}"`));
...
async function run(targets) {
    ...
    console.log(`\n[+] Done. ${stats.total} targets | ${stats.passwordlessFound} passwordless accounts found | ${stats.errors} errors`);
}
